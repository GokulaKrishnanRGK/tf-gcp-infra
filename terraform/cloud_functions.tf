resource "google_cloudfunctions2_function" "verify_email_function" {
  ...
  service_config {
    ...
    service_account_email          = google_service_account.logger_account[count.index].email
    environment_variables = {
      CSYE6225_SQL_DB_CONNSTR = "jdbc:mysql://${google_sql_database_instance.database_instance[count.index].ip_address.0.ip_address}/${google_sql_database.database[count.index].name}"
      CSYE6225_SQL_DB_USER    = google_sql_user.users[count.index].name
      CSYE6225_SQL_DB_PWD     = google_sql_user.users[count.index].password
